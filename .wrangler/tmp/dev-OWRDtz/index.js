        ...
        const filteredResults = results.map((row) => {
          try {
            ...
            return {
              ...
              message: parsed.body,
              ttl: parsed.ttl,
              createdAt: (/* @__PURE__ */ new Date(row.created_at + " UTC")).getTime()
            };
          } catch (e) {
            return { nickname: row.nickname, message: row.message, ttl: 3600, createdAt: 0 };
          }
        }).filter((item) => {
          return item.createdAt + item.ttl * 1e3 > now;
        });
...
// .wrangler/tmp/bundle-j8I7zt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-j8I7zt/middleware-loader.entry.ts
